...
import {
  compareStringIndices, findNearestRacketNode, Atom,
  Token, ParserStatus, Parser
} from '../parser';

describe('Internal function', () => {
  describe(compareStringIndices.name, () => {
    it('Should return false when the first pair is null', () => {
      assert.strictEqual(compareStringIndices(
        null,
        { index: 1, length: 4 }
      ), false);
    });
  ...
  describe(findNearestRacketNode.name, () => {
    ...
    const casesMap: Map<string, Token> = new Map();
    casesMap.set('#| <> |#', {
      step: 'BlockComment',
    ...
    casesMap.forEach((expected: Token, src: string) => {
